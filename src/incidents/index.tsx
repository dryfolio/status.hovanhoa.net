				...
				<div>
					{(monthlyIncidents as MonthlyIncident[]).map((incidents) => (
						<div className="mb-10" key={incidents.month}>
							<p className="mr-5 text-2xl font-semibold leading-6 text-[var(--rd-text)]">
								{incidents.month}
							</p>
							<div className="mt-2 flex-1 h-px bg-[var(--rd-border)]" />
							<div className="ml-6 border-l-4 border-[var(--rd-border)]">
								{(incidents.incidents as Incidents[]).map((incident) => (
									<div className="flex" key={incident.id}>
										<div className="-ml-4 mt-6 flex rounded-full w-7 h-7 bg-[var(--rd-surface-2)]">
											<svg
												...
												viewBox="0 0 14 14"
												fill="none"
												className="m-auto fill-[var(--rd-text-3)] text-disrupted"
											>
										...
										<div className="items-center ml-3 mt-6">
											<p className="text-base font-semibold leading-6 text-[var(--rd-text)]">
												{incident.title}
											</p>
